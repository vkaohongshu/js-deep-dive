...
// map 实现
function myMap<T>(arr: T[], callback: (item: T, index: number, array: T[]) => void, that?: any): any[] {
...
// reduce 实现
function myReduce<T>(arr: T[], callback: (prev: any, cur: T) => void, initVal?: any) {
    if (arr === null || arr === undefined || !Array.isArray(arr)) {
...
function myPush<T>(arr: T[], ...items: T[]) {
    const O = Object(arr);
